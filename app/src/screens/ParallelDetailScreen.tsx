...
export default function ParallelDetailScreen() {
  ...
  try { diffAnnotations = JSON.parse(entry.diff_annotations_json || '[]'); } catch { /* */ }

  // Build a map from book name to Gospel display name for diff labels
  const bookToName = new Map<string, string>();
  for (const p of passages) {
    const cfg = GOSPEL_CONFIG[p.book];
    bookToName.set(p.book, cfg?.name ?? p.book.charAt(0).toUpperCase() + p.book.slice(1));
  }
