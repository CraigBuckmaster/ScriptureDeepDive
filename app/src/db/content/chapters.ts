...
export async function getChapterById(id: string): Promise<Chapter | null> {
  return getDb().getFirstAsync<Chapter>('SELECT * FROM chapters WHERE id = ?', [id]);
}

export async function getDifficultyForBook(bookId: string): Promise<Map<number, number>> {
...
export async function getVHLGroups(chapterId: string): Promise<VHLGroup[]> {
  return getDb().getAllAsync<VHLGroup>('SELECT * FROM vhl_groups WHERE chapter_id = ?', [
    chapterId,
  ]);
}

export async function getRedLetterVerses(bookId: string, chapterNum: number): Promise<number[]> {
...
export async function getProofTextGuard(
  bookId: string,
  chapterNum: number,
  verseNum: number,
): Promise<ProofTextGuard | null> {
  try {
    return await getDb().getFirstAsync<ProofTextGuard>(
      `SELECT * FROM proof_text_guards
