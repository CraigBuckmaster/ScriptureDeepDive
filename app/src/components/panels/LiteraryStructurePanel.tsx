    ...
    <View style={{ gap: spacing.sm }}>
      {data.rows.map((row, i) => (
        <View key={i} style={{
          ...
          <Text style={{ color: colors.accent, fontFamily: fontFamily.uiSemiBold, fontSize: 12, minWidth: 55 }}>
            {row.label}
          </Text>
      ...
      {data.note ? (
        <Text style={{ color: base.textMuted, fontFamily: fontFamily.bodyItalic, fontSize: 13, marginTop: spacing.xs }}>
          {data.note}
        </Text>
      ) : null}
    </View>
