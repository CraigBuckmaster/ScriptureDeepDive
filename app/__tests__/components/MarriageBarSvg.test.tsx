...
const makeBar = (overrides?: Partial<{ x1: number; x2: number; y: number; midX: number; partnerId: string; spouseId: string }>) => ({
  x1: 100,
  x2: 200,
  y: 50,
  midX: 150,
...
describe('MarriageBarSvg', () => {
  ...
  it('applies reduced opacity when dimmed', () => {
    const { UNSAFE_getByType } = renderWithProviders(<MarriageBarSvg bar={makeBar()} dimmed />);
    const g = UNSAFE_getByType('G' as any);
    expect(g.props.opacity).toBe(0.1);
  });

  it('applies normal opacity when not dimmed', () => {
    const { UNSAFE_getByType } = renderWithProviders(<MarriageBarSvg bar={makeBar()} />);
    const g = UNSAFE_getByType('G' as any);
