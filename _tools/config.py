"""
config.py — Data constants for the Companion Study build system.

This file contains pure data (no functions, no side effects). It defines:
  - COMMENTATOR_SCOPE: Which scholars cover which books (controls panel generation)
  - BOOK_META: Per-book metadata (author info, VHL highlight word lists)
...
# ── Per-book metadata ────────────────────────────────────────────────────
# Used by save_chapter() to populate VHL (Verse Highlight) groups and
# by the app to display book-level author/date information.
#
# Fields:
#   is_nt       — True for New Testament, False for Old Testament
#   auth        — HTML string shown on the Book Intro screen (author, date, context)
#   vhl_places  — Place names highlighted in verse text (tappable → map)
#   vhl_people  — Person names highlighted in verse text (tappable → genealogy tree)
#   vhl_time    — Time words highlighted in verse text (tappable → timeline)
#   vhl_key     — Key theological terms highlighted in verse text (tappable → word study)
# ─────────────────────────────────────────────────────────────────────────
BOOK_META = {
    'genesis': {
        'is_nt': False,
...
# ── Scholar Registry ─────────────────────────────────────────────────────
# Canonical list of all scholars used in the app's commentary panels.
# Adding a new scholar means:
#   1. Add entry here
#   2. Add to COMMENTATOR_SCOPE above
#   3. Add color to app/src/theme/colors.ts
#   4. Add label to app/src/utils/panelLabels.ts
#   5. Add bio data to content/meta/scholar-bios.json + scholar-data.json
#
# Tuple fields:
#   data_key     — key used in section data dicts (e.g. 'mac', 'netbible')
#   scope_key    — key used in COMMENTATOR_SCOPE (e.g. 'macarthur')
#   label        — display name shown on panel buttons in the app
#   panel_suffix — suffix for panel IDs (e.g. 'mac' → {section_id}-mac)
#
SCHOLAR_REGISTRY = [
...
# Wave 7: Revelation scholars
SCHOLAR_REGISTRY.append(('beale', 'beale', 'Beale', 'beale'))
SCHOLAR_REGISTRY.append(('osborne', 'osborne', 'Osborne', 'osborne'))
COMMENTATOR_SCOPE['beale'] = ['revelation']
COMMENTATOR_SCOPE['osborne'] = ['revelation']

BOOK_META['revelation'] = {
    'is_nt': True,
    'auth': "<strong>Author:</strong> John, who identifies himself four times (1:1, 4, 9; 22:8). Early church tradition unanimously identified him as John the apostle, son of Zebedee (Justin Martyr, Irenaeus, Clement of Alexandria, Tertullian). Dionysius of Alexandria (mid-3rd c.) first questioned apostolic authorship on stylistic grounds, noting the Greek of Revelation is rougher than the Fourth Gospel. Modern scholarship remains divided: conservative scholars generally maintain apostolic authorship; critical scholars often posit a separate 'John of Patmos' or 'John the Elder.' The author's intimate knowledge of the OT (over 500 allusions), his authority over the Asian churches, and his self-designation as a fellow-sufferer all point to a figure of considerable apostolic stature.\\n\\n<strong>When written:</strong> Two main dates are proposed. The <em>early date</em> (c. AD 64-68) places composition during Nero's persecution, supported by some internal evidence (the temple still standing in ch. 11, the number 666 as a Nero cipher). The <em>late date</em> (c. AD 95-96) during Domitian's reign is the majority position, supported by Irenaeus's explicit testimony, the developed state of the Asian churches, and the imperial cult context that pervades the book. Most scholars favor the Domitianic date.\\n\\n<strong>What prompted it:</strong> John writes from exile on Patmos (1:9) to seven churches in the Roman province of Asia facing a constellation of threats: imperial persecution, pressure to participate in emperor worship, internal false teaching (Nicolaitans, 'Jezebel,' Balaamite compromise), spiritual complacency, and the seductive prosperity of Roman civilization. The book unveils the true nature of the cosmic conflict between God and Satan, assures believers that Christ has already won the decisive victory through his death and resurrection, and reveals the certain consummation of God's redemptive plan: the destruction of all evil, the vindication of the martyrs, and the descent of the New Jerusalem as the eternal dwelling of God with his people. Revelation is simultaneously a letter (chs. 1-3), a prophecy (1:3; 22:7), and an apocalypse (1:1)—the only NT book that combines all three genres.",
    'vhl_places': ['Patmos', 'Ephesus', 'Smyrna', 'Pergamum', 'Thyatira', 'Sardis', 'Philadelphia', 'Laodicea', 'Jerusalem', 'Babylon', 'Armageddon', 'Mount Zion', 'Euphrates River', 'Egypt', 'Sodom'],
    'vhl_people': ['Jesus', 'John', 'Satan', 'Michael', 'Balaam', 'Jezebel', 'Moses', 'Abraham', 'David', 'Elijah', 'Antipas'],
